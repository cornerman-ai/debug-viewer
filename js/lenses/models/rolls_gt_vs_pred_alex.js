// Rolls GT vs pred (Alex) — the punch classifier's GT-vs-Pred lens, for rolls:
// John's labeled rolls against Rolly's HELD-OUT predictions (LightGBM, 5-fold
// grouped by video), with the per-frame p(roll) graph and live decode sliders.
//
// Identical in every way to ./rolls_gt_vs_pred_mathe.js — same schema, same decode /
// matching math — just pointed at a different model's dump, so the two lenses are
// directly comparable. Keep the two in sync if the shared logic changes.
//
// Data: lens_data/roll_detector_alex/ (index.json + one JSON per round), written by
//   cd ~/code/cornerman-rolls/model && python export_lens_data.py \
//     --model rolly --run rolly1_higher_capacity \
//     --out ~/code/cornerman-debug-viewer/lens_data/roll_detector_alex
// That model is 3-class (nothing / lead_roll / rear_roll); the p(roll) plotted here is
// P(lead_roll) + P(rear_roll), the roll-vs-nothing view the Mathe detector predicts
// directly, so Pred carries no lead/rear distinction on either lens.
//
// Schema:
...
const DATA_DIR = "./lens_data/roll_detector_alex/";
const GRID_FPS = 30;
const IOU_MATCH = 0.5;
...
// ── mount / template ────────────────────────────────────────────────────────
export const RollsGtVsPredAlexRule = {
  id: "rolls_gt_vs_pred_alex",
  label: "Rolls GT vs pred (Alex)",
  ...
  mount(_host, state) {
    ...
    ensureIndex().then(() => {
      ...
      if (sel && sel.value && sel.value !== "rolls_gt_vs_pred_alex") return;
      renderRun();
      populateRoundPicker();
      docKey = null;
...
function template() {
  return `
    ...
    <p class="hint">Below the video: GT rolls (green = found · yellow-green = center-hit only ·
      ...
      and the p(roll) graph with the threshold line. Click to seek · drag to select a range (then Export /
...
function renderRun() {
  ...
  if (indexError) {
    el.innerHTML = `<span style="color:${COLORS.gtMiss}">${DATA_DIR}index.json failed to load (${indexError}).</span> ` +
      `Write it with an evaluator script, <code>--lens-out …/lens_data/roll_detector_alex</code>`;
    return;
  }
